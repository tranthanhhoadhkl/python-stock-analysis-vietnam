...
def fetch_vnindex_data(days: int = 400) -> pd.DataFrame:
    """
    Lấy dữ liệu VN-Index từ API TCBS (nguồn dữ liệu VN chính thức).
    Trả về DataFrame với cột Date, Close. Nếu thất bại trả về DataFrame rỗng.
    """
    import requests
    try:
        end_dt = datetime.now()
        start_dt = end_dt - timedelta(days=days)
        # TCBS stock history API – dùng mã VNINDEX
        url = (
            "https://apipubaws.tcbs.com.vn/stock-insight/v1/stock/bars-long-term"
            f"?ticker=VNINDEX&type=index&resolution=D"
        ...
        # Cột: tradingDate (ms epoch hoặc ISO), open, high, low, close, volume
        if "tradingDate" in df.columns:
            df["Date"] = pd.to_datetime(df["tradingDate"], unit="ms", errors="coerce")
            if df["Date"].isna().all():
                df["Date"] = pd.to_datetime(df["tradingDate"], errors="coerce")
...
def fetch_vnindex_ssi(days: int = 400) -> pd.DataFrame:
    """Backup: lấy VN-Index từ SSI iBoard API."""
    import requests
    try:
        end_dt = datetime.now()
        start_dt = end_dt - timedelta(days=days)
        url = (
            "https://iboard-query.ssi.com.vn/v2/stock/bars"
            f"?symbol=VNINDEX&resolution=D"
            f"&from={int(start_dt.timestamp())}&to={int(end_dt.timestamp())}"
        )
        resp = requests.get(url, timeout=10,
                            headers={"User-Agent": "Mozilla/5.0",
        ...
        resp.raise_for_status()
        data = resp.json()
        bars = data.get("data", data.get("items", []))
        if not bars:
            return pd.DataFrame()
        df = pd.DataFrame(bars)
        if "t" in df.columns:
            ...
            df["Date"] = pd.to_datetime(df["tradingDate"], errors="coerce")
        col_close = next((c for c in ["c", "close", "Close"] if c in df.columns), None)
        if not col_close:
            return pd.DataFrame()
...
# Cache đơn giản để tránh gọi API quá nhiều lần trong một session
_vnindex_cache: Dict[str, Any] = {}

def get_vnindex_df(days: int = 400) -> pd.DataFrame:
    """Lấy dữ liệu VN-Index với fallback: TCBS → SSI → rỗng."""
    cache_key = f"vni_{days}"
    if cache_key in _vnindex_cache:
        cached_time, cached_df = _vnindex_cache[cache_key]
        if (datetime.now() - cached_time).seconds < 300:  # cache 5 phút
            return cached_df
    df = fetch_vnindex_data(days)
    if df.empty:
        df = fetch_vnindex_ssi(days)
    _vnindex_cache[cache_key] = (datetime.now(), df)
    return df


def get_vnindex_benchmark(days: int = 365) -> Dict[str, Any]:
    """Lấy benchmark VN-Index (return%, chart) — không dùng yfinance."""
    try:
        df_vn = get_vnindex_df(days + 30)
        if df_vn.empty or len(df_vn) < 2:
            return {"return_pct": 0.0, "chart": [], "available": False}
        close = df_vn["Close"].values
...
async def get_dashboard():
    ...
    # Market-wide regime dựa trên VN-Index
    market_regime = "ĐI NGANG"
    try:
        df_market = get_vnindex_df(90)
...
async def get_stock_analysis(symbol: str, timeframe: str = "7"):
    ...
    # Beta
    try:
        df_vn_beta = get_vnindex_df(fetch_days + 90)
        if not df_vn_beta.empty:
            df_vn_beta["Ret"] = df_vn_beta["Close"].pct_change()
            df_merged = pd.merge(
                df_fe[["Date", "Return_1d"]].rename(columns={"Return_1d": "stock_ret"}),
                df_vn_beta[["Date", "Ret"]].rename(columns={"Ret": "mkt_ret"}),
                on="Date", how="inner"
            )
...
async def compare_stocks(symbols: str):
    ...
    for sym in sym_list:
        try:
            ...
            try:
                df_vn_cmp = get_vnindex_df(425)
                if not df_vn_cmp.empty:
                    df_vn_cmp["Ret"] = df_vn_cmp["Close"].pct_change()
                    df_merged = pd.merge(
                        df_fe[["Date", "Return_1d"]].rename(columns={"Return_1d": "stock_ret"}),
                        df_vn_cmp[["Date", "Ret"]].rename(columns={"Ret": "mkt_ret"}),
                        on="Date", how="inner"
                    )
                    beta = compute_beta(df_merged["stock_ret"], df_merged["mkt_ret"])
                else:
